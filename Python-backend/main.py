...
def get_speech_rate(y, sr):
    # Approximate speech rate as number of energy bursts per second
...
def _sync_process_file(contents):
    ...
    pitch_value = get_pitch_librosa(y, sr)
    energy_value = get_energy(y)
    tone_value = get_tone(y, sr)
    speech_rate_value = get_speech_rate(y, sr)
    ...
    result = {
        ...
        "speech_rate": {"value": round(speech_rate_value,2), "unit":"events/sec", "label": speech_rate_label(speech_rate_value)},
